...
### 添加与图标
- 🎨 **水彩手绘图标**：277 个预设食材/日用图标（Gemini 批量生成 + sharp/WebP 压缩）。*方向调整中：转「日式淡雅」绘本气质重制，详见 [project-design.md](docs/project-design.md) §8.7*
- 🤖 **AI 生成图标**：预设没有的物品可现场 AI 生成（Edge Function → Gemini），全局限额
- 🗂️ **账号图标库**：AI 图标归属账号、家人并集共享，可从「全家图库」借用
- 🟫 **长尾兜底**：没有对应图片的商品用**纯色块**（按名字 hash 落暖色盘）——语义诚实、克制（*决策已定待实现；此前经历 灰块→食物小人→梅兰竹菊装饰系 三次换轨，见 §8*）
...
- 🎨 **图标 / 收藏方向调整中（2026-07-08 定，见 [ROADMAP](docs/ROADMAP.md) 进行中 + [§8.7](docs/project-design.md)）**：① 长尾兜底改**纯色块**；② 梅兰竹菊 12 花转做**收藏系统「钤印集章」**（完成采购随机钤朱红印记 → 集章本翻面看回忆 → 节气限定印）；③ 277 预设转**日式淡雅**重制
- 📋 上架后验证：北美华人家庭种子用户裂变假设、W1/W4 留存
- ⚠️ **仓库已从 OneDrive 桌面挪到 `C:\dev\MaiSha`**（OneDrive KFM 同步会搅乱大二进制/git，导致文件错乱、commit 丢失）——后续开发在新路径进行
