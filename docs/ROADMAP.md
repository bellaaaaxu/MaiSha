...
三处叙事分裂（宪法「双人共享」/ repositioning「Global 个人优先」/ §9「北美华人滩头」）统一为叙事金字塔：**L1 个人手账 → L2 家庭放大 → L3 华人深度（获客显性、产品普适）**。宪法 §1 与本页定位简述已重写；App Store 三语元数据草案定稿在 spec 附录 C；app 内两处 i18n 文案见待办。
...
- **app 内叙事文案落地** — 两个 i18n key：`onboarding.slogan`（你的买菜手账 · 按店记，不漏买）+ `share.description`（把清单发给家人，点开就能一起勾——不用注册）；三语终稿在 [统一叙事 spec](superpowers/specs/2026-07-02-unified-narrative-design.md) §4.4/§4.5。
