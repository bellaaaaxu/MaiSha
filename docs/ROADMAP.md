...
- **长尾兜底换装：梅兰竹菊装饰系 + 首字角标 — spec 已立（2026-07-05），待实施** — 弊端 #1/#2 的正式处置：描绘→装饰框架换轨，12 只无脸雅集花卉（等权重 rendezvous，id 永不改名单向门沿袭）+ 右下角首字纸片角标补识别；无晕染底（与 Tier 1 裸图同语言）；食物小人退役至品牌层/图鉴层（public/mascots 移出 precache，定稿留 mascot-staging/final 备图鉴）。试装批 4 只已过目定稿（梅v1/兰v1/竹v1/菊v3，兰待「花大叶少」重生成一轮），剩 8 只出图。宪法 §8 已同步修订。
  - spec: [superpowers/specs/2026-07-05-flora-decor-fallback-design.md](superpowers/specs/2026-07-05-flora-decor-fallback-design.md)
...
- **⚠️ 班底已知弊端 #1：撞脸是常态不是运气差** — 11+1 池按生日悖论，一张清单 5–6 件长尾商品出现同脸概率过半（2026-07-05 实测 7 件长尾三件同为茶叶蛋）；队长 weight 2 = 17.5% 长尾都是小榕包，双队长同框常见。同脸商品间图标失去区分度，购物模式打勾有「凭图标点错行」摩擦。扩池 32 砍半但消不掉；零成本逃生通道备选：「长按小人换一只」写进现有自定义图标指派层（存 `/mascots/x.webp` 当自定义图标，零 schema 变更）。**处置（2026-07-05）**：flora spec 定案——装饰框架下撞图=贴纸复用非 bug，且首字角标恢复逐件区分；spec 落地后本条关闭
- **⚠️ 班底已知弊端 #2：语义化分配的决策窗口正在关闭（单向门）** — 「同一商品永远同一只脸」与「按品类挑成员」（创可贴→医疗角色）根本冲突：rendezvous 扩池时新成员只能随机接走 ~1/13 商品，**无法定向**；且手输商品 db 写死 `category='其他'`，没有品类信号可路由。想上语义分层（或整体替换兜底方案）必须趁种子期做——**当前替换成本为零（班底层不写任何 DB，纯渲染层），用户量起来后「不换脸」承诺生效，永久关闭**。附带弊端备忘：食物脸贴非食品的违和（泻药/杀虫剂配笑脸点心涉「危险品画成食物」之忌）、萌兜底可能挤掉用户自助补图动力（无埋点不可观测）。**处置（2026-07-05 定案）**：梅兰竹菊无脸装饰系 + 首字角标（[flora spec](superpowers/specs/2026-07-05-flora-decor-fallback-design.md)）——装饰不声称品类，语义路由问题随框架消解，本条随 spec 落地关闭；版权排雷结论：节气/麻将传统纹样/市花/邮票框均安全（自绘原创即可），POI 有剪影商标风险不做
