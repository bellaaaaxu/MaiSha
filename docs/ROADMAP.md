...
- **设置区死链修复（2026-07-02 新发现 bug，上架阻塞）** — SettingsDrawer「语言设置」「导入 / 导出」导航到不存在的路由 `/settings/language`、`/settings/import-export`，被 `*` 兜底**静默弹回 /list**：上完 onboarding 就再也改不了语言（三语 app 的硬伤）。顺带处置孤儿路由 `/settings`（Settings.tsx 全页硬编码中文、无任何入口引用，功能已被找回码卡片 / 头部分享 / 抽屉覆盖——删除或重建为真设置页，需 brainstorm 定方向）。
