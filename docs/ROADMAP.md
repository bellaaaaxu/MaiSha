...
### 🆕 收藏系统「钤印集章」v1（2026-07-19 落地）

Subagent-driven 全流程交付（7 任务，每任务实现+规格审+质量审两道关）：**完成采购随机钤一枚朱红花印 → 集章本翻面看首钤回忆 → 季节限定窗**。migration 015 `seal_collection`（account 级、RLS 无 DELETE=印记不可逆）；`pickSeal` rendezvous 发放（8 常驻均匀 + 水仙/荷/桂/梅四季窗必得、已拥有 50% 再钤、梅窗跨年，5 单测）；`SealImprint` 用现有 flora webp CSS mask 转朱红 `#B0442C`（**零新资产**）；ShoppingEndModal 保存成功后揭晓（压下-顿-抬起动画 + `@capacitor/haptics` 触觉 web 降级；**先落库再演出、失败宁缺毋滥**）；`/seals` 集章本（stagger 点亮 + 数字滚动 + card-flip 首钤三件套 + 空印晃动 + 季节印「到季开钤」预告）。**186 测试全过**；e2e 冒烟（真实两轮采购）：季节窗必得（7-19 得荷）→ 已拥有落常驻（第二轮得菊）→ 翻面「2026/7/19 · 在未指定店铺买了 2 样」精确 → locked 恰好 3 枚 → 空印晃动 → 零 console 错误；DB 行形状逐字段核对。

- spec: [superpowers/specs/2026-07-19-seal-collection-design.md](superpowers/specs/2026-07-19-seal-collection-design.md) · plan: [superpowers/plans/2026-07-19-seal-collection.md](superpowers/plans/2026-07-19-seal-collection.md)
- 审查流程战果：测试断言公式复刻(T2)、SELECT 漏 error 检查(T3)、**Windows cap sync 写坏 iOS SPM 路径**(T5)、spec 点名的空印晃动缺失(T6)、揭晓页导航竞态(T7 e2e 实测 nav(-1) 与 push 互踩)——全部审出并修复
- ⚠️ **踩坑备忘：Windows 上跑 `npx cap sync` 会把 `ios/App/CapApp-SPM/Package.swift` 的 SPM 路径写成反斜杠（Mac 上必编译失败）**——每次 sync 后须检查该文件并改回正斜杠再提交
- 待办延后：真机触觉体感（并入上架前 iOS 冒烟）；「无 account 完成采购」边缘走代码守卫（`if (account)`）+ 双审确认，未做真机 e2e

### 🆕 长尾兜底换装：梅兰竹菊装饰系 + 首字角标（2026-07-05 落地）

食物小人上线当日发现结构性弊端后火线换轨（描绘→装饰框架，弊端两条随之关闭）：12 只无脸雅集花卉（梅兰竹菊松荷桂银杏枫水仙紫藤芦苇）等权重 rendezvous 分配 + 右下角首字纸片角标（ZCOOL KuaiLe 手写体，识别功能拿回）；无晕染底，与 Tier 1 裸图同语言。资产 27 张候选零失败、零二轮重生成（兰按「花大叶少」重生成一次到位），12 张 WebP q80 共 161KB（松针纹理密，验收线 150→161 接受）。小人代码删除、public/mascots 退役出 precache（定稿留 mascot-staging/final 备图鉴）。172 测试全过（166−7 小人+13 花），preview 冒烟：花+角标就位、预设优先级不破坏、零 console 错误。

- spec: [superpowers/specs/2026-07-05-flora-decor-fallback-design.md](superpowers/specs/2026-07-05-flora-decor-fallback-design.md) · plan: [superpowers/plans/2026-07-05-flora-decor-fallback.md](superpowers/plans/2026-07-05-flora-decor-fallback.md)
- 宪法 §8 已同步修订；成员 id 永不改名单向门沿袭（扩池只增不改）
- 2026-07-07 用户冒烟反馈 hotfix ×4：① **示例商品跟随语言重写**——种子是建单时的数据行，切语言后不再滞留旧语言（`relocalizeExampleItems` 纯函数：只精确匹配种子名/提示文案含全部历史变体，用户改过名绝不触碰；副作用红利：切回中文自动重新命中写实预设图标）；② **备注 chip 加宽 60→100px**（三语原提示在 9px 字号下全部被截——「Tap the c…」什么都教不会）；③ **提示文案改指真实动作**「去购物试试打勾」——旧文案「点(左边)圆圈打勾」描述的圆圈只存在于购物模式，清单页物品只有改/删（用户指出）；备注跟随商品进购物模式行内，新文案两个页面都读得通。178 测试全过（+6），preview 实测切换重写 + legacy 同语言就地升级；④ **字标换霞鹜文楷 TC，繁体恢复「買啥」**——根因：「買」不在站酷快乐体（简体字库）里，浏览器混入系统黑体导致字标一半手绘一半黑体（用户指出突兀）。首版对策「字标固定简体买啥」被用户否决，改为换字体：**LXGW WenKai TC（SIL OFL，简繁拉丁全覆盖手写楷体）**，经四候选对比表用户钦点。落地：Google Fonts `text=` 子集只载「买/啥/買」三字形（几 KB）+ 专用 `--font-wordmark` 变量（不动 `--font-title`——它被店名/页标广泛使用，整体换会在词中混手）+ 字标 700 加粗；两个字标位（头部 title + onboarding Wordmark）生效，三语字标同一只手
- 2026-07-07 附带修复：vite `server.host: true` 双栈监听——Node ≥17 默认只绑 ::1，浏览器把 localhost 解析成 127.0.0.1 时连接被拒（用户实遇）；局域网开放顺带服务未来 iOS 真机冒烟
- 2026-07-08 用户决定 **删除示例商品的「去购物试试打勾」引导备注**——示例清单本身即引导，不需要再教「打勾」（此前三轮都在打磨这条文案，用户追问后决定直接去掉）。`SEEDS` 去 hint 字段、`buildExampleItems` 首件不再带 note；`relocalizeExampleItems` 改为「切语言时把任一历史引导备注一律清空」（含全部旧文案）；后端一次性 PATCH 清掉已存在的 4 条（service role，只清已知引导文案、用户自写备注不碰）。179 测试全过，preview 实测新建清单无备注 + 页面「打勾」字样归零

### 🆕 食物小人班底·最小版（长尾图标）（2026-07-05 落地）

长尾商品兜底从灰色 CSS 色块升级为「食物小人 + 晕染底」：10 只常驻（饺子/糖葫芦/煎饼果子/茶叶蛋/蛋挞/菠萝包/虾饺/鸡蛋仔/珍珠奶茶/凤梨酥）+ 队长小榕包 weight 2 加权。分配用 **rendezvous/HRW 一致性哈希**（FNV-1a + fmix32 终混——裸 FNV 对虚拟节点雪崩不足会让队长加权失效，测试逮到后修正），锁死「扩池老商品不换脸」单向门。出图走 API 批量（`generate-mascots.mjs`，30 张候选零失败，按定妆铁律筛 10）；压缩管线升级为**边缘取样自适应底色**（出图底色白到茶色不等，固定阈值漏抠），11 张 WebP 共 99KB 入 precache。图片加载失败自动回退水彩文字 blob。166 测试全过（+7），preview 冒烟：预设优先级不破坏（洗洁精命中预设）、长尾见小人、IconPickerPanel「使用文字」预览同步换小人。

- plan: [superpowers/plans/2026-07-04-mascot-troupe-minimal.md](superpowers/plans/2026-07-04-mascot-troupe-minimal.md) · 设计 [project-design.md](project-design.md) §8
- 决策：全量 32 只不作为上架门槛（§9.5 精简上架）；季节限定 4 只留作收藏品图鉴稀缺款不进池
- 备忘：成员 id 永不改名（分配依赖）；App Store 截图现在**可以**出现小人了

### 🆕 找回/加入链路 i18n 收编 + recovery 文案打磨（2026-07-03 落地）

上架前「recovery 文案过一眼」的审计结果：文案本身基本清晰，但**整条找回/加入链路是硬编码简体**（RecoveryCodeCard、JoinByCode 全页、List 空状态与 Onboarding 的两个找回入口）——英文/繁体用户在最关键的数据安全路径上会看到简体。已全部收编 i18n（新增 `recoveryCard`/`joinPage` 两节 + `common.close`，三语齐）；顺手把找回副标题改为「输入你之前记下的找回码」（告诉用户码从哪来）。159 测试全过，preview 实测 zh/en 双语切换命中。
...
- **梅兰竹菊装饰池扩容（12 → 16+）** — 种子验证跑 1–2 周后，用 Supabase `items` 表商品名离线跑一遍 icon-registry 匹配量出真实长尾 fallback 命中率（现有埋点拿不到分布：add_item 事件不含商品名），再决定扩多少；候选雅集词汇：山茶/牡丹（圆瓣团型与菊互斥，至多再取一）、海棠（枝花型与梅互斥）、芭蕉、蒲公英；rendezvous 保证扩池老商品不换贴纸
- **⚠️ 班底已知弊端 #1：撞脸是常态不是运气差** — 11+1 池按生日悖论，一张清单 5–6 件长尾商品出现同脸概率过半（2026-07-05 实测 7 件长尾三件同为茶叶蛋）；队长 weight 2 = 17.5% 长尾都是小榕包，双队长同框常见。同脸商品间图标失去区分度，购物模式打勾有「凭图标点错行」摩擦。扩池 32 砍半但消不掉；零成本逃生通道备选：「长按小人换一只」写进现有自定义图标指派层（存 `/mascots/x.webp` 当自定义图标，零 schema 变更）。**已关闭（2026-07-05 flora 落地）**：装饰框架下撞图=贴纸复用非 bug，首字角标恢复逐件区分
- **⚠️ 班底已知弊端 #2：语义化分配的决策窗口正在关闭（单向门）** — 「同一商品永远同一只脸」与「按品类挑成员」（创可贴→医疗角色）根本冲突：rendezvous 扩池时新成员只能随机接走 ~1/13 商品，**无法定向**；且手输商品 db 写死 `category='其他'`，没有品类信号可路由。想上语义分层（或整体替换兜底方案）必须趁种子期做——**当前替换成本为零（班底层不写任何 DB，纯渲染层），用户量起来后「不换脸」承诺生效，永久关闭**。附带弊端备忘：食物脸贴非食品的违和（泻药/杀虫剂配笑脸点心涉「危险品画成食物」之忌）、萌兜底可能挤掉用户自助补图动力（无埋点不可观测）。**已关闭（2026-07-05 flora 落地）**：装饰不声称品类，语义路由问题随框架消解；版权排雷结论存档：节气/麻将传统纹样/市花/邮票框均安全（自绘原创即可），POI 有剪影商标风险不做
- **收藏品（图鉴）** — 2026-07-05 方向更新：主机制改为**节气采购章**——完成采购时按当值节气（节气=24 个区间覆盖全年每一天，无空窗）在采购结束弹窗/采购历史条目盖章；「集齐 24 节气章」的收集动作与北极星指标（每周完成采购）**直接咬合**，一年一轮天然稀缺、季节限定免费送。决策依据：清单是规划物不是日记流，添加日期无意义（周一~五加周六买→全清单同章；月长旅行清单→无意义分组），**唯一有灵魂的日期是完成采购那天**——商品图标层刻意无时间（名字 hash 稳定分配），时间语义全部归采购章。食物小人班底萌物 + 季节限定款（青团/粽子/月饼/汤圆）保留作图鉴角色层，与吉祥物共用资产（刻意只上 10 只常驻——全量日常曝光透支稀缺感）；onboarding 稳定后单独 brainstorm
...
- **IconPickerPanel 文案收编 i18n** — 「使用文字/实物拍摄/AI 生成/免费」等硬编码简体（班底上线后「使用文字」的语义也已变成「交给小伙伴」，文案顺带打磨）；与剩余弹窗清扫同批做
