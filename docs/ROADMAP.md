...
- **⚠️ 班底已知弊端 #2：语义化分配的决策窗口正在关闭（单向门）** — 「同一商品永远同一只脸」与「按品类挑成员」（创可贴→医疗角色）根本冲突：rendezvous 扩池时新成员只能随机接走 ~1/13 商品，**无法定向**；且手输商品 db 写死 `category='其他'`，没有品类信号可路由。想上语义分层（或整体替换兜底方案）必须趁种子期做——**当前替换成本为零（班底层不写任何 DB，纯渲染层），用户量起来后「不换脸」承诺生效，永久关闭**。附带弊端备忘：食物脸贴非食品的违和（泻药/杀虫剂配笑脸点心涉「危险品画成食物」之忌）、萌兜底可能挤掉用户自助补图动力（无埋点不可观测）
