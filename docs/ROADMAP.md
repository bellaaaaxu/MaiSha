...
- 2026-07-07 用户冒烟反馈 hotfix ×3：① **示例商品跟随语言重写**——种子是建单时的数据行，切语言后不再滞留旧语言（`relocalizeExampleItems` 纯函数：只精确匹配种子名/提示文案含全部历史变体，用户改过名绝不触碰；副作用红利：切回中文自动重新命中写实预设图标）；② **备注 chip 加宽 60→100px**（三语原提示在 9px 字号下全部被截——「Tap the c…」什么都教不会）；③ **提示文案改指真实动作**「去购物试试打勾」——旧文案「点(左边)圆圈打勾」描述的圆圈只存在于购物模式，清单页物品只有改/删（用户指出）；备注跟随商品进购物模式行内，新文案两个页面都读得通。178 测试全过（+6），preview 实测切换重写 + legacy 同语言就地升级
