...
- 2026-07-07 用户冒烟反馈 hotfix ×2：① **示例商品跟随语言重写**——种子是建单时的数据行，切语言后不再滞留旧语言（`relocalizeExampleItems` 纯函数：只精确匹配种子名/提示文案含历史长文案，用户改过名绝不触碰；副作用红利：切回中文自动重新命中写实预设图标）；② **备注 chip 加宽 60→92px + 种子提示缩短**（三语原文案在 9px 字号下全部被截——「Tap the c…」什么都教不会）。177 测试全过（+5），preview 双向切换实测
