...
- **留存埋点 + Sentry 错误监控 — 代码已实现并合并（2026-07-03）** — `events` 表（migration 014：RLS 只进不出 + `env` 列隔离 dev/prod）+ 三个分析视图（北极星=每周完成采购的清单数 / 周活 / W1·W4 留存 cohort，仅统计 prod）；五个核心事件全部接线：`add_item` / `complete_trip` / `share_link_open`（ua_env 自动识别微信 webview）/ `list_join` / `store_finder_used`；Sentry errors-only、仅生产构建初始化。149 单测全过；preview 实测事件请求发出、远端表未建时 404 静默且主流程零受影响；隐私政策页同步四处更新（含修正「不追踪地理位置」与查超市定位权限的失实冲突）。
  ...
  - 2026-07-03 部署进展：migration 014 已 push ✓（顺带 repair 迁移历史表——001–012 此前系 Dashboard 手工应用、历史表为空，已标记 applied，今后 `db push` 可正常使用）；**dev 环境端到端实测通过**：事件 `POST /rest/v1/events → 201` 入库（`env='dev'`，被分析视图排除，不污染留存数据）。
  - 2026-07-03 Sentry 已上线：DSN 配在 Workers **Build** 变量（注意：不是顶部的运行时 Variables 区）；踩坑记录——加完变量后 Retry deployment 没让新变量生效，需**全新构建**（push 触发）才注入；已 curl 线上 bundle 实证 `ingest.us.sentry.io` 在场。
  - **剩余（用户侧最后一步）**：无痕窗口打开线上站 → console 抛测试错 → Sentry Issues 见条目 ✓；顺手加一件商品 → Dashboard 跑 `select name, env from events order by id desc limit 5;` 应见 `env='prod'` 行 ✓——两个都见到，本条目移入「已上线」。
